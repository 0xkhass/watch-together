...
export function VoiceChat({
  ...
  const { userId, username } = useRoomStore();
            ...
            <AnimatePresence>
              {voiceUserList.map(user => (
                <VoiceUserCard
                  ...
                  isCurrentUser={false}
                  onVolume={(v) => onPeerVolume(user.userId, v)}
                />
              ))}
            </AnimatePresence>
      ...
      {isInVoice && (
        <div className="p-3 border-t border-white/[0.06] flex gap-2">
          ...
          <button
            type="button"
            onClick={onLeave}
            className="flex items-center justify-center px-4 py-2 rounded-xl text-sm font-medium bg-rose-500/20 text-rose-400 border border-rose-500/30"
          >
