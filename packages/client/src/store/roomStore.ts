...
interface RoomStore {
  ...
  setUserId: (id: string) => void; // <--- 1. ADD THIS TO THE INTERFACE
  ...
  // ... (rest of your interface stays exactly the same) ...
...
export const useRoomStore = create<RoomStore>((set, get) => ({
  ...
  // <--- 2. ADD THIS FUNCTION TO THE STORE
  setUserId: (id) => {
    localStorage.setItem(STORAGE_KEYS.USER_ID, id);
    set({ userId: id });
  },
  ...
  clearRoom: () => set({ room: null, currentMember: null, messages: [], reactions: [] }),

  // ... (the rest of your implementations below stay exactly the same!) ...
